    ...
    function(require, module, exports) {
      ...
      var exports = function() {
        ...
        Camera.prototype.init = function(rad1, rad2, width, height) {
          ...
          this.rad1 = rad1;
          this.rad2 = rad2;
          this.obj = new THREE.PerspectiveCamera(35, this.width / this.height,
              1, 10000);
    ...
    function(require, module, exports) {
      var exports = function() {
        ...
        Get.prototype.pointSphere = function(rad1, rad2, r) {
          var x = Math.sin(rad1) * Math.sin(rad2) * r;
          var y = Math.cos(rad2) * r;
    ...
    function(require, module, exports) {
      ...
      var exports = function() {
        ...
        HemiLight.prototype.init = function(
            scene, rad1, rad2, r, hex1, hex2, intensity) {
          this.r = r;
...
// ==========
      ...
      var init = function() {
        ...
        light = new HemiLight();
        light.init(scene, get.radian(45), get.radian(45), 1000, 0xffffff,
            0x000000, 1);
        ...
        for (var i = 0; i < boxArr.length; i++) {
          var r = get.randomInt(64, 255);
          var g = get.randomInt(64, 255);
          var b = get.randomInt(0, 128);
          var verticesLength = boxVertices.length;
          var facesLength = boxFaces.length;

          for (var index = 0; index < boxFaces.length; index++) {
            indicesArr[facesLength * 3 * i + index * 3 +
            0] = boxFaces[index].a + i * verticesLength;
            indicesArr[facesLength * 3 * i + index * 3 +
            1] = boxFaces[index].b + i * verticesLength;
            indicesArr[facesLength * 3 * i + index * 3 +
            2] = boxFaces[index].c + i * verticesLength;
          }
          for (var index = 0; index < boxVertices.length; index++) {
            verticesArr[verticesLength * 3 * i + index * 3 +
            0] = boxVertices[index].x + boxArr[i].position.x;
            verticesArr[verticesLength * 3 * i + index * 3 +
            1] = boxVertices[index].y + boxArr[i].position.y;
            verticesArr[verticesLength * 3 * i + index * 3 +
            2] = boxVertices[index].z + boxArr[i].position.z;
        ...
        bufferGeometry.addAttribute('index',
      ...
      var render = function() {
        ...
        for (var i = 0; i < boxArr.length; i++) {
          ...
          for (var index = 0; index < boxVertices.length; index++) {
            var vector = boxVertices[index].clone().
            applyQuaternion(boxArr[i].q);
            verticesArr[verticesLength * 3 * i + index * 3 + 0] = vector.x +
                boxArr[i].position.x;
            verticesArr[verticesLength * 3 * i + index * 3 + 1] = vector.y +
                boxArr[i].position.y;
            verticesArr[verticesLength * 3 * i + index * 3 + 2] = vector.z +
                boxArr[i].position.z;
          }
        }
    ...
    function(require, module, exports) {
      ...
      var exports = function() {
        ...
        Mover.prototype.updateQuaternion = function() {
          ...
          var cross = new THREE.Vector3().crossVectors(this.up, dir).
          normalize();
    ...
    function(require, module, exports) {
      ...
      var exports = function() {
        ...
        PointLight.prototype.init = function(
            scene, rad1, rad2, r, hex, intensity, distance) {
          this.r = r;
